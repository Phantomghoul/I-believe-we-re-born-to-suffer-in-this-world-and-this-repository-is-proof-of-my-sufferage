...
void solve() {
    ...
    // get the students are that selected
    for(int i =0; i<n;i++){
        //perform the check
        if(selected[s[i]-1].second < p[i]){
            selected[s[i]-1].first = i+1;
            selected[s[i]-1].second = p[i];

        }
    }

    //find out the students who aren't selected and add them to a new school
    //simply count the ones that aren't selected and subtract by k. k - count
    int cnt = 0;
    for(int i =0; i <k;i++){
        for(int j = 0; j <m;j++){
            if(selected[j].first == c[i])cnt++;
        }
    }
...
/*
    observations
    - We know the power of each student
    - We also know the school of each student
    - We can easily see what student will get selected
    - there's 3 cases that might happen
	    all students that are chosen will be selected
	    a few students would be selected
	    no students would be selected
    - we can use a vector to keep track of the school and pair to track their id and power.
    - for each student, we file them in their school and compare each students power to the current highest power student in the school in the vector, if the student's power is greater than the student in the current place than we get their id and power and place them in the school. if not then we cycle through the next student

    - we now check if any chosen ones are selected, we can simply count which are and aren't.
    - note that the powers are distinct.
*/
/*
